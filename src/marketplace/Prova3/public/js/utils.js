...
// ─── PAGINATOR ──────────────────────────────────────
function renderPaginazione(containerId, paginaCorrente, totalePagine, callbackCambia) {
  const container = document.getElementById(containerId);
  if (!container || totalePagine <= 1) { if (container) container.innerHTML = ''; return; }
...
function lunghezzaInMinuti(lunghezza) {
  const mappa = { '3s': 0.05, '15s': 0.25, '30s': 0.5, '1m': 1, '3m': 3, '5m': 5, '10m': 10 };
