...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Ricerca catalogo
  document.getElementById('cercaCatalogo')?.addEventListener('input', debounce(e => {
...
async function caricaAutori() {
  ...
  utenti.filter(u => ['autore','admin'].includes(u.ruolo)).forEach(u => {
    const opt = document.createElement('option');
    opt.value = u._id; opt.textContent = `${u.username} (${u.ruolo})`;
    sel.appendChild(opt);
  });
...
async function caricaVisiteList() {
  ...
  list.innerHTML = data.visite.map(v => `
...
function renderCatalogo(filtro = '') {
  ...
  if (!items.length) {
    container.innerHTML = '<div class="aa-empty" style="padding:1rem"><p style="font-size:0.8rem">Nessun item trovato.</p></div>';
    return;
  }
...
function iconaCategoriaPiccola(cat) {
  const m = { pittura:'🖼️',scultura:'🗿',architettura:'🏛️',fotografia:'📷',arte_moderna:'🎨',arte_antica:'🏺',decorativa:'🪆',altro:'🔍' };
  return m[cat] || '🔍';
}
...
function renderPercorso() {
  ...
  document.getElementById('countItems').textContent = `${itemsNelPercorso.length} item`;

  // Calcola durata totale
  const durataMin = itemsNelPercorso.reduce((acc, i) => acc + lunghezzaInMinuti(i.lunghezza), 0);
  document.getElementById('durataCalcolata').textContent = `Durata: ~${Math.round(durataMin)} min`;
  ...
  list.innerHTML = itemsNelPercorso.map(item => `
...
function onDragEnd(e) {
  ...
  document.querySelectorAll('.aa-dnd-item').forEach(el => el.style.borderTop = '');
}
...
async function salvaVisita() {
  ...
  const tags      = document.getElementById('visitaTags').value.split(',').map(t=>t.trim()).filter(Boolean);
  ...
  if (!titolo || !museo || !autoreId) return showToast('Compila i campi obbligatori (Titolo, Museo, Autore)', 'error');
  if (!itemsNelPercorso.length) return showToast('Aggiungi almeno un item al percorso', 'error');

  const payload = {
    titolo, museo, descrizione: desc, tags,
...
async function caricaVisitaPerModifica(id) {
  ...
  document.getElementById('visitaLicenza').value     = v.licenza?.tipo || 'gratuito';
