...
exports.getItems = async (req, res) => {
  try {
    const {
      museo, linguaggio, categoria, licenza, minPrezzo, maxPrezzo,
      cerca, pagina = 1, limite = 20, pubblicato = 'true'
    } = req.query;
...
exports.getItemById = async (req, res) => {
  try {
    const item = await Item.findById(req.params.id).populate('creatorId', 'username');
...
exports.creaItem = async (req, res) => {
  try {
    ...
    if (!utente || !['autore', 'admin'].includes(utente.ruolo)) {
      return risposta(res, 403, null, 'Solo gli autori possono creare contenuti');
    }
    ...
    if (err.name === 'ValidationError') {
      return risposta(res, 400, null, Object.values(err.errors).map(e => e.message).join(', '));
    }
    risposta(res, 500, null, err.message);
  }
};
...
exports.getVisite = async (req, res) => {
  try {
    const { museo, creatorId, pubblica = 'true', pagina = 1, limite = 12 } = req.query;
    const filtro = {};
    if (museo) filtro.museo = museo;
    ...
    const [visite, totale] = await Promise.all([
      Visita.find(filtro)
        .populate('creatorId', 'username')
        .populate('items.itemId', 'titolo operaId lunghezza linguaggio immagine')
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(Number(limite)),
...
exports.getVisitaById = async (req, res) => {
  try {
    const visita = await Visita.findById(req.params.id)
      ...
      .populate({
        path: 'items.itemId',
        select: 'titolo operaId lunghezza linguaggio immagine descrizione autore categoria prezzo licenza'
      });
...
exports.getStats = async (req, res) => {
  try {
    const [totItems, totVisite, totUtenti, itemsGratuiti, itemsAPagamento] = await Promise.all([
    ...
    risposta(res, 200, { totItems, totVisite, totUtenti, itemsGratuiti, itemsAPagamento });
  } catch (err) {
    risposta(res, 500, null, err.message);
  }
};