...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  configuraBtnFiltri('[data-museo]',  v => { stato.filtri.museo = v;      stato.paginaItems = 1; caricaItems(); });
  configuraBtnFiltri('[data-lang]',   v => { stato.filtri.linguaggio = v; stato.paginaItems = 1; caricaItems(); });
  configuraBtnFiltri('[data-cat]',    v => { stato.filtri.categoria = v;  stato.paginaItems = 1; caricaItems(); });
  configuraBtnFiltri('[data-prezzo]', v => { stato.filtri.prezzo = v;     stato.paginaItems = 1; caricaItems(); });

  // Ricerca testuale con debounce
  document.getElementById('campoCerca')?.addEventListener('input', debounce(e => {
  ...
  if (stato.filtri.museo) {
    setTimeout(() => {
      const btn = document.querySelector(`[data-museo="${stato.filtri.museo}"]`);
      if (btn) {
        document.querySelectorAll('[data-museo]').forEach(b => b.classList.remove('active'));
...
function configuraBtnFiltri(selector, callback) {
  document.querySelectorAll(selector).forEach(btn => {
    btn.addEventListener('click', () => {
      btn.closest('div').querySelectorAll('.aa-filter-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      callback(btn.dataset.museo ?? btn.dataset.lang ?? btn.dataset.cat ?? btn.dataset.prezzo ?? '');
    });
  });
}
...
function switchTab(tab, btnEl) {
  ...
  document.getElementById('tabItems').classList.toggle('d-none', tab !== 'items');
  document.getElementById('tabVisite').classList.toggle('d-none', tab !== 'visite');
...
async function caricaMuseiFiltro() {
  ...
  musei.forEach(m => {
    ...
    btn.title = m;
    btn.addEventListener('click', () => {
      container.querySelectorAll('.aa-filter-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
...
async function caricaItems(pagina = stato.paginaItems) {
  ...
  grid.innerHTML = '<div class="col-12 text-center py-5"><div class="aa-spinner"></div> Caricamento...</div>';
  ...
  if (!data) { grid.innerHTML = '<div class="col-12"><div class="aa-empty"><div class="aa-empty-icon">❌</div><p>Errore nel caricamento.</p></div></div>'; return; }

  if (!data.items.length) {
    grid.innerHTML = '<div class="col-12"><div class="aa-empty"><div class="aa-empty-icon">🔍</div><h5>Nessun risultato</h5><p>Prova a modificare i filtri di ricerca.</p></div></div>';
...
function iconaCategoria(cat) {
  const mappa = {
    pittura:'🖼️', scultura:'🗿', architettura:'🏛️', fotografia:'📷',
    arte_moderna:'🎨', arte_antica:'🏺', decorativa:'🪆', altro:'🔍'
  };
  return mappa[cat] || '🔍';
}
...
async function apriItemModal(id) {
  ...
  document.getElementById('modalItemBody').innerHTML = '<div class="text-center py-4"><div class="aa-spinner"></div></div>';
  document.getElementById('modalItemFooter').innerHTML = '';

  const item = await apiFetch(`/api/items/${id}`);
  if (!item) { modal.classList.add('d-none'); return; }
...
async function caricaVisiteTab(pagina = stato.paginaVisite) {
  ...
  grid.innerHTML = '<div class="col-12 text-center py-5"><div class="aa-spinner"></div></div>';
  ...
  if (!data || !data.visite.length) {
    grid.innerHTML = '<div class="col-12"><div class="aa-empty"><div class="aa-empty-icon">🗺️</div><h5>Nessuna visita disponibile</h5><p>Crea la prima visita dall\'Editor.</p><a href="/editor-visita" class="btn-aa-primary mt-2">Crea Visita</a></div></div>';
    return;
  }

  grid.innerHTML = data.visite.map(v => renderVisitaCard(v)).join('');
  renderPaginazione('paginazioneVisite', pagina, data.pagine, 'caricaVisiteTab');
}
...
async function caricaMieiContenuti() {
  ...
  container.innerHTML = '<div class="text-center py-4"><div class="aa-spinner"></div></div>';
  ...
  if (['autore', 'admin'].includes(u.ruolo)) {
    ...
    if (!data) return;
    const miei = data.items.filter(i => i.creatorId?._id === u._id || i.creatorId === u._id);
...
async function apriLogModal() {
  ...
  body.innerHTML = '<div class="text-center py-4"><div class="aa-spinner"></div></div>';

  const items = await apiFetch('/api/log/vendite');
  if (!items || !items.length) {
    body.innerHTML = '<div class="aa-empty"><div class="aa-empty-icon">📋</div><p>Nessuna vendita registrata.</p></div>';
    return;
  }
...
function resetFiltri() {
  stato.filtri = { museo:'', linguaggio:'', categoria:'', prezzo:'', cerca:'' };
  stato.paginaItems = 1;
